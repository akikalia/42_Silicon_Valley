...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/04/19 06:15:12 by akikalia          #+#    #+#             */
/*   Updated: 2019/04/19 06:15:14 by akikalia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_ssl.h"

static unsigned int g_t[] = {0, 0xd76aa478, 0xe8c7b756, 0x242070db,
...
void	abcd(t_words *w, unsigned int k, unsigned int s, unsigned int i)
{
	w->a = w->b + rotate_left((w->a +
				(*w->func)(w->b, w->c, w->d) + (w->x)[k] + g_t[i]), s);
}

void	dabc(t_words *w, unsigned int k, unsigned int s, unsigned int i)
{
	w->d = w->a + rotate_left((w->d +
				(*w->func)(w->a, w->b, w->c) + (w->x)[k] + g_t[i]), s);
}

void	cdab(t_words *w, unsigned int k, unsigned int s, unsigned int i)
{
	w->c = w->d + rotate_left((w->c +
				(*w->func)(w->d, w->a, w->b) + (w->x)[k] + g_t[i]), s);
}

void	bcda(t_words *w, unsigned int k, unsigned int s, unsigned int i)
{
	w->b = w->c + rotate_left((w->b +
				(*w->func)(w->c, w->d, w->a) + (w->x)[k] + g_t[i]), s);
}
